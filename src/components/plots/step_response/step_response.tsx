...
export default function StepResponse({ t_max }: { t_max: number }) {
  ...
  async function handlePlot() {
    try {
      let response;
      if (type === "tf")
        response = await transfer_function_api.step({
          ...state,
          t_max,
          x_axis,
          y_axis,
        ...
        response = await state_space_api.step({
          ...state,
          t_max,
          x_axis,
          y_axis,
  ...
  async function fetchPerformanceSpecs() {
    try {
      let response;
      if (type === "tf")
        response = await transfer_function_api.step_performance(state);
      else response = await state_space_api.step_performance(state);
